...
//                this.$Message.info('开关状态：' + status);
      ...
      getlist (page) {
        ...
        uAxios.get('admin/paas/workers?page=' + page + '&keyword=' + self.searchKeyword + jump)
          .then(res => {
            ...
            self.information = result.data.map((item) => {
              return {
                ...
                rank: item.user.rank,
                is_good_match: item.user.is_good_match == '0' ? '未认证' : '已认证',
                invite_user_num: item.invite_user_num
              }
            })
